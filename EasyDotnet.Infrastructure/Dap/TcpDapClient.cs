...
public class TcpDapClient(ILogger<TcpDapClient> logger, Action<string> callback) : IDisposable
{
  ...
  /// </summary>
  public async Task StartAndConnect(IPAddress address, int port, Func<Task>? onClientDisconnected = null, TimeSpan? timeout = null)
  {
    ...
    try
    {
      _client = await _listener.AcceptTcpClientAsync()
                               .WaitAsync(timeout ?? TimeSpan.FromSeconds(30), _cts.Token);
    }
