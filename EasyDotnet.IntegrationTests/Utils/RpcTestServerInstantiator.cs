...
public static class RpcTestServerInstantiator
{
  ...
  public static async Task<JsonRpc> GetInitializedStreamServer()
  {
    ...
    await server.InvokeWithParameterObjectAsync<TestInitializeResponse>("initialize", new List<TestInitializeRequest>() { new(InitializeTests.DummyTestInfo, new TestProjectInfo(Path.GetTempPath())) });
    return server;
  }
