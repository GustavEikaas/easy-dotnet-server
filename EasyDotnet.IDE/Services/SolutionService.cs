...
{
  public async Task<SolutionModel> GetSolutionModelAsync(string solutionFilePath, CancellationToken cancellationToken)
  {
    ...
    var serializer = SolutionSerializers.GetSerializerByMoniker(fullSolutionPath)
        ?? throw new InvalidOperationException($"No serializer found for solution file: {fullSolutionPath}");
    return await serializer.OpenAsync(fullSolutionPath, cancellationToken);
  }
  ...
  public async Task<List<SolutionFileProject>> GetProjectsFromSolutionFile(string solutionFilePath, CancellationToken cancellationToken)
  {
    ...
    var solutionModel = await GetSolutionModelAsync(fullSolutionPath, cancellationToken);
